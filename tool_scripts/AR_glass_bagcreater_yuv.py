...
def loadImageToRosMsg(filename):
    ...
    #image format is '.bmp', '.png', '.jpg', ".pgm"
        ...
        timestamp_nsecs = os.path.splitext(os.path.basename(filename))[0]
        timestamp = rospy.Time( secs=int(timestamp_nsecs[0:-9]), nsecs=int(timestamp_nsecs[-9:]))
        #timestamp_second = timestamp.to_sec()
        #timestamp_second = timestamp_second + 5e-3
        #timestamp = rospy.Time.from_sec(timestamp_second)
...
def createAccGyroMessge(timestamp_int, omega, alpha):
    ...
    acc_msg.vector.x = float(alpha[0])
    acc_msg.vector.y = float(alpha[1])
    acc_msg.vector.z = float(alpha[2])

    gryo_msg.header.stamp = timestamp
    gryo_msg.vector.x = float(omega[0])
    gryo_msg.vector.y = float(omega[1])
    gryo_msg.vector.z = float(omega[2])
