...
namespace aslam
{
  ...
  namespace cameras
  {
    ...
    {
      return _fu == rhs._fu && _fv == rhs._fv && _cu == rhs._cu && _cv == rhs._cv && _ru == rhs._ru && _rv == rhs._rv && _recip_fu == rhs._recip_fu && _recip_fv == rhs._recip_fv && _fu_over_fv == rhs._fu_over_fv && _distortion.isBinaryEqual(rhs._distortion);
    }
    ...
    bool PinholeProjection<DISTORTION_T>::initializeIntrinsics(const std::vector<GridCalibrationTargetObservation> &observations)
    {
      ...
      for (size_t i = 0; i < nImages; ++i)
      {
        ...
        {
          for (size_t k = j + 1; k < centers.size(); ++k)
          {
            ...
            ipts = PinholeHelpers::intersectCircles(centers[j](0), centers[j](1),
                                                    radiuses[j], centers[k](0), centers[k](1), radiuses[k]);
            if (ipts.size() < 2)
              continue;
        ...
        double &outErr) const
    {
      ...
      for (size_t i = 0; i < obs.target()->size(); ++i)
      {
        ...
        if (obs.imagePoint(i, y) && euclideanToKeypoint(T_camera_target * obs.target()->point(i),
                                                        yhat))
        {
          outErr += (y - yhat).norm();
          ++count;
        ...
        sm::kinematics::Transformation &out_T_t_c) const
    {
      ...
      for (size_t i = 0; i < Ms.size(); ++i)
      {
        ...
        if (keypointToEuclidean(imagePoint, backProjection) && backProjection.normalized()[2] > std::cos(80.0 * M_PI / 180.0))
        {
